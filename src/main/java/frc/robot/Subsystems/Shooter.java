...
public class Shooter extends SubsystemBase {
  ...
  public void periodic() {
    ...
    simField.getObject("ShotTraj").setPose(driveTrainPos.plus(new AdvancedPose2D(turretAimVector.getX(), turretAimVector.getY())));
    simField.getObject("Target").setPose(new AdvancedPose2D(targetPose.getX(), targetPose.getY()));
    simField.getObject("BotVelocity").setPose(driveTrainPos.plus(new AdvancedPose2D(driveTrainSpeeds.vxMetersPerSecond, driveTrainSpeeds.vyMetersPerSecond)));
    simField.getObject("TurretVelocity").setPose(currentPosition.plus(new AdvancedPose2D(currentVelocity.getX(), currentVelocity.getY())));
    SmartDashboard.putString("TurVel", "(" + currentVelocity.getX() + ", " + currentVelocity.getY() + ")");
    simField.getObject("TurPos").setPose(currentPosition);

    SmartDashboard.putData("ShooterSimField", simField);
  }
